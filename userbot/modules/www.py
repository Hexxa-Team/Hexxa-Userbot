...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("`.....🔥Hexxa-Userbot🔥.....`")
    await pong.edit("`🔥`")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"•⎚• ⎆ __Hexxa__ **Pong!**\n"
                    f"☞  __Ping:__ "
                    f"`%sms` \n"
                    f"☞  __Sisa Waktu:__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("**Memulai Test Kekuatan Jaringan**")
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"卍**TEST PING HEXXA-USERBOT**卍\n"
                    f"═⎆ **Ping:** "
                    f"`%sms` \n"
                    f"═⎆ **Sisa Waktu:** "
                    f"`{uptime}` \n"
...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("__Hexxa__")
    await pong.edit("__PONG__")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"┏━━| **Hexxa PONG** |━━卍\n"
                    f"┣|•  __Ping:__ "
                    f"`%sms` \n"
                    f"┗|• __Uptime:__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    await pong.edit("⎚⎆ __Hexxa__ **Test Ping!**\n`%sms`" % (duration))
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Perintah :** `!ping`\
        \n  •  **Function : **Pingnya hampir sama dengan ultroid namun ini versi Hexxa-Userbot!\
    "
    }
)
