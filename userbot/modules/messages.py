...
async def fastpurger(purg):
    ...
    if purg.reply_to_msg_id is not None:
        ...
        return await purg.edit("`Mohon Balas Ke Pesan Hexxa `")

    if msgs:
        await purg.client.delete_messages(chat, msgs)
    done = await purg.client.send_message(
        purg.chat_id, f"`Berhasil Menghapus Pesan Hexxa`\
        \nJumlah Pesan Yang Dihapus {str(count)} Pesan")
    """
    if BOTLOG:
        await purg.client.send_message(
            BOTLOG_CHATID,
            "Berhasil Menghapus Pesan Hexxa " + str(count) + " Pesan Berhasil  Dibersihkan.")
    """
    await sleep(2)
    await done.delete()
...
async def purgeme(delme):
    ...
    smsg = await delme.client.send_message(
        delme.chat_id,
        "`Berhasil Menghapus Pesan Hexxa,` " + str(count) + " `Pesan Telah Dihapus`",
    )
    """
    if BOTLOG:
        await delme.client.send_message(
            BOTLOG_CHATID,
            "`Hexxa Telah Menghapus Pesan,` " + str(count) + " Pesan Telah Dihapus`")
    """
    await sleep(2)
    i = 1
    await smsg.delete()
...
async def delete_it(delme):
    ...
    if delme.reply_to_msg_id:
        try:
            ...
            if BOTLOG:
                await delme.client.send_message(
                    BOTLOG_CHATID, "`Hexxa Berhasil Menghapus Pesan`")
            """
